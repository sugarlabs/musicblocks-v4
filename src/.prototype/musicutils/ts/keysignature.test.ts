...
function compareScales(scale1Arg: Array<string>, scale2Arg: Array<string>): boolean {
    const scale1 = scale1Arg.slice(0, -1);
    scale1.sort();
    const scale2 = scale2Arg.slice(0, -1);
    scale2.sort();
...
describe('Key Signature', () => {
    ...
    test('invert transforms', () => {
        ...
        expect(ks2.get_scale().length).toBe(8);
        expect(ks2.get_scale()[7]).toBe('n18');
    ...
    test('Mode equivalents', () => {
        ...
        const source = ks.normalizeScale(ks.scale);
        ks = new KeySignature('minor', 'db');
        let target = ks.normalizeScale(ks.scale);
        expect(compareScales(source, target)).toBe(true);
