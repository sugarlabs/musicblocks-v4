...
export default class SynthUtils implements ISynthUtils {
    ...
    constructor() {
        ...
        for (let i = 0; i < SAMPLES.length; i++) {
            import SAMPLES[i] from '../samples/' + SAMPLES[i];
            this.samples[SAMPLES[i]] = SAMPLES[i];
        }
    ...
    private _addSynths(instance: number) {
        ...
        for (const sample in this.samples) {
            ...
            const _data = this.samples[sample]['function']();
            _synth = new Tone.Sampler({ _pitch: _data });
            samplerMap.set(sample, _synth);
            // } else {
            //    _synth = new Tone.Player(this.samples[sample]['function']());
    ...
    /**
      ...
      */
    public getBuiltinSynth(instrumentName: string, instance: number = 0): Tone.PolySynth | undefined {
        const instanceName = instance.toString();
    ...
    /**
      ...
      * @throws {InvalidArgumentError}
      */
    public getSampler(instrumentName: string, instance: number = 0): Tone.Sampler | undefined {
        const instanceName = instance.toString();
    ...
    /**
      ...
      * @throws {InvalidArgumentError}
      */
    public getPlayer(instrumentName: string, instance: number = 0): Tone.Player | undefined {
        const instanceName = instance.toString();
     ...
     **/
    public noteValueToSeconds(noteValue: number, instance: number = 0): number {
        const instanceName = instance.toString();
    ...
    /**
      ...
      * @throws {InvalidArgumentError}
      */
    public trigger(pitches: (string|number)[], noteValue: number, instrumentName: string, instance: number = 0) {
        const instanceName = instance.toString();
        const now = Tone.now();
        /**
         * We calculate an offset based on how many notes we have already played.
         */
        const offset = this.noteValueToSeconds(this.notesPlayed[instance]);
        ...
        let _synth;
        if (instrumentName in this.builtinSynths[instanceName]) {
            _synth = this.builtinSynths[instanceName].get(instrumentName);
            if (_synth !== undefined) {
            ...
            _synth = this.samplers[instanceName].get(instrumentName);
            if (_synth !== undefined) {
                console.log(typeof(_synth));
                Tone.loaded().then(() => {
            ...
            _synth = this.players[instanceName].get(instrumentName);
            if (_synth !== undefined) {
                console.log(typeof(_synth));
                Tone.loaded().then(() => {
                    _synth.start();
