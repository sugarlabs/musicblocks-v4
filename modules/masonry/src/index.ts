...
function _generateNotchRight(config: {
    bBoxArgs: TBBox[];
    bBoxLabel: TBBox;
    strokeWidth: number;
    notchSpacing?: number;
}): { path: string[]; vertical: number } {
    const { bBoxArgs, bBoxLabel, strokeWidth, notchSpacing } = config;

    if (bBoxArgs.length === 0) {
        return { path: [], vertical: 0 };
    }

    const requiredMinimum = strokeWidth / 2 + Math.max(MIN_LABEL_HEIGHT, bBoxLabel.h) + strokeWidth / 2;
    ...
    for (let i = 0; i < bBoxArgs.length; i++) {
        ...
        const connectingBrickTransitions =
        ...
        const variableLength = Math.max(
            0,
            totalRequiredForThisArg - totalFixedHeight,
        );
        ...
        path = path.concat(notchPath);
        vertical += 4 + (-3) + 10 + (-3) + 4; // Only verticals matter
...
function _generateRight(config: {
    ...
    if (hasArgs) {
        const notchRight = _generateNotchRight({ bBoxArgs, bBoxLabel, strokeWidth });
        path.push(...notchRight.path);
        vertical += notchRight.vertical;
    } else {
        const totalHeight =
            strokeWidth / 2 + Math.max(MIN_LABEL_HEIGHT, bBoxLabel.h) + strokeWidth / 2 - CORNER_RADIUS * 2;
        const rightEdge = Math.max(totalHeight, HEIGHT_NOTCH_RIGHT);
        path.push(`v ${rightEdge}`);
        vertical += rightEdge;
    }
...
function _generateNestedPath(config: {
    ...
    function _generateNestedBottom(): string[] {
        ...
        const variableOuterWidth =
            variableWidth + WIDTH_NOTCH_TOP - strokeWidth / 2;
        ...
        return [
            ...
            ...(hasNotch
                ? _generateNotchBottom(strokeWidth)
                : [`h -${WIDTH_NOTCH_BOTTOM}`]),
    ...
    return [
        ..._generateInnerLeft(),
        ..._generateNestedBottom(),
        ..._generateNestedOuterLeft(),
    ];
}
